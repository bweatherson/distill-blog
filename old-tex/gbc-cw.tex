\chapter{Games, Beliefs and Credences}

\newcommand{\RG}{{\sc Red-Green}}

\pubdata{\textit{Philosophy and Phenomenological Research} 92 (2016): 209-36. I've discussed this paper with many people. I'm especially grateful to feedback from Jessica Brown, Josh Dever, Ishani Maitra, Lisa Miracchi, Sarah Moss, Levi Spectre and Jacob Ross, and the audience at the Rutgers Formal Epistemology Workshop/Epistemology Conference.}
...
%\section{Regularity and the Lockeans}
%
%There are several theses that could deserve the name \textbf{Regularity}. Here are three. In all three, the quantifier ranges over any rational agent $S$, and any proposition $p$ that the agent regards as being a live possibility.
%
%\begin{enumerate}
%\item $S$ gives $p$ a positive real credence.
%\item $S$ gives $p$ a numerical credence greater than zero.
%\item $S$ gives $p$ a credence greater than zero.
%\end{enumerate} 
%
%\noindent The theses are arranged in decreasing order of strength; the first entails the second and the second the third.  But the second could be true without the first, if we allow the agent to have infinitesimal credences, and the third could be true without the second, if we allow purely comparative, non-numerical credences.
%
%The first claim is pretty clearly implausible. If there are continuum many live possibilities, the agent can't give each a positive real credence. And such cases seem clearly possible.
%
%The second is more interesting, but I think Timothy \citet{Williamson2007-WILHPI} has a compelling argument against it. I'm going to very briefly go over Williamson's argument, and then argue (\textit{contra} Williamson) that it doesn't extend to an argument against the third form of regularity. In the following section, I'll offer a game theoretic argument against the third form, and hence against any variety of the Lockean thesis.
%
%Williamson's argument concerns infinite series of coin flips. Our agent $S$ knows that a fair coin will be tossed infinitely often, one toss after the other. (How could she know this? Perhaps she knows that each toss will take half as often as the one before it. This is, to put it mildly, physically implausible in a world like ours, but that doesn't seem relevant to investigating a conceptual claim like regularity.) Let $p$ be the proposition that the coin will never land tails, although it is flipped infinitely often. What credence should the agent give to $p$, assuming this credence must be numerical, need not be real (i.e., it could be infinitesimal), and must be greater than zero?
%
%Williamson argues that there is no good answer to this question. The argument has, as I read it, two key steps. 
%
%The first step imagines that there are two distinct coins that are both going to be flipped infinitely often, and the coins are both fair, indeed they are qualitative duplicates. Let $c_1$ be the agent's credence that the first will never land tails, while $c_2$ is the agent's credence that the second will never land tails. Williamson's first premise is $c_1 = c_2$. How might we defend that premise? I can think of three reasons; the first of which on reflection isn't any good, but the other two are better.
%
%We might try reasoning to a limit. For any finite $n$, the probability that the first coin doesn't land tails on its first $n$ flips is equal to the probability that the second coin doesn't land heads on its first $n$ flips. Could we assume this equality will also hold in the limit? That seems rash; lots of strange things happen at the limit.
%
%We might try reasoning from the assumption that the probabilities are numerical. If $c_1$ and $c_2$ are not equal, and both of them are numerical, then one of them must be greater. But the symmetry of the situation makes that implausible. So they are equal. This seems like a convincing reason to me, but note we've relied on the assumption that $c_1$ and $c_2$ are numerical.
%
%Alternatively, and this is closer to Williamson's own argument, we might reason from the symmetry of the situation to the equality of the probabilities. Maybe the qualitative identity of the coin flipping situations implies that the probabilities of all heads for each is the same. I don't think this quite works as it stands. The symmetry of the flipping situations should imply that the probabilities of all heads on each are symmetric. But we need another step to show that they should be equal. If the probabilities are all numerical, then the only way for them to be symmetric is for them to be identical. For now we are assuming numerical probabilities, so it is harmless to infer that $c_1$ and $c_2$ are identical. But note that we need this assumption to get the conclusion of equality.\footnote{I've argued at length against the confusion of symmetric probabilities with equal probabilities in \citet{Weatherson2005-WEASWR}.}
%
%For the second step, imagine that the first of these coins was tossed once before the infinite sequence started. Let $c_0$ be the agent's credence that coin never landed tails either during the infinite sequence, or in this initial toss. Since the probability that the initial toss is heads is $\nicefrac{1}{2}$, and this toss is independent of the sequence to follow, it should be that $c_1 = 2c_0$. But since the `initial' toss plus the sequence to follow is an initial sequence, it follows from the first step that $c_2 = c_0$. And since $c_1 = c_2$, we conclude that $c_1 = 2c_1$. And from that we can conclude that $c_1 = 0$, even if $c_1$ is allowed to be infinitesimal.
%
%So far, this argument strikes me as basically sound, at least as an argument against the second form of regularity. But I don't think it should be particularly worrying to the theorist who wants to defend the third form of regularity. And that's because once we drop the assumption that $c_1$ and $c_2$ are numerical, then we can't take as a premise that $c_1 = c_2$. Williamson has an extended argument that shows that once we accept $c_1 = c_2$, and accept a form of the sure-thing principle for non-numerical probabilities, it follows that $c_1 = 0$. (That is, it follows that the probability of all heads is no greater than the probability of a contradiction.) We could try quibbling with the forms of sure-thing principle Williamson uses\footnote{Indeed, this is what is suggested by \citet{HaverkampSchulz2012}.}, but I think they are sound principles and have no interest in quibbling.
%
%Instead, I think we could simply deny the first premise. If $c_1$ is neither greater than, nor less than, not equal to, $c_2$, then it is consistent with every other premise Williamson puts forward that both $c_1$ and $c_2$ are greater than zero. That is, $c_1$ and $c_2$ could be \textit{incomparable} with each other, while both being greater than zero.
%
%Why should we believe this incomparability claim? Well, if we were independently committed to regularity, then it seems by far the best way to save at least one form of regularity. And if we believe the Lockean thesis, then we need regularity. On the other hand, if was an independent reason to reject regularity, and with in the Lockean thesis, then we would have no reason to say that $c_1$ and $c_2$ are incomparable. I'll turn now to such a reason.
...
%
%1. Iterated 'probably' supports content over attitude view
%2. These are odd contents. Must be person/time (or perhaps evidence) relative, but feels like we can preserve them. Maybe that's enough.
%3. Need something to explain why we're less and less disposed to rely on $p$ as evidence against it comes in.
%4. A primitive binary relation of \textit{is more confident that} would help a lot with this problem. And it would be enough for all the credences we need.
%5. If we take the binary relation \textit{is more confident that} to be prior to the graded relation \textit{is confident to degree x that}, then credences become somewhat epiphemonenal. But they are still a useful shorthand.
%6. Credences (or something like them) can provide both a causal explanation for why agents abandon beliefs, and provide a standard of correctness for abandoning beliefs.
%
...
Another reason comes from recent work by Jessica \citet{Brown2013}. Compare these two situations.
...
At heart, the theory I've offered here is simple. To believe \emph{p} is to take \emph{p} for granted, to take it as given, to take it as a settled question. But one doesn't take a question as settled in a vacuum. I will take some questions as settled in some circumstances and not others. It's here that the complexities enter in.
