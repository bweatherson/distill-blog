...
\pubdata{\textit{Synthese} 166 (2009): 333-357. Thanks to Herman Cappelen, Andy Egan, Anthony Gillies, John Hawthorne, Jonathan Ichikawa, Jeff King, Ishani Maitra, Jason Stanley, Zoltan Szab\'{o}, Crispin Wright and audiences at the 2005 LOGOS workshop in Barcelona, and at Michigan, Rutgers and St Andrews.}
\par}
...
First, there is some epistemic relation R such a proposition \textit{s} is among the X iff some salient individual \textit{i} stands in relation R to \textit{s}. We'll use R(\textit{i}) to represent those propositions. It will become important later that X is genuinely a plural variable, so R(\textit{i}) is not a set of propositions, or a fusion of propositions (whatever that would be). Rather, I just mean to be plurally referring to the propositions that stand in relation R to \textit{i}. (Note that I'm not saying anything here about how \textit{i} is determined; my preferred theory is that it is the \textit{evaluator} of any conditional utterance, but nothing in the simple epistemic theory turns on this.)
...
On the other hand, there had been some discussions of the position I'm calling `indexical relativism'. In \citet{Egan2005-EGAEMI} we call such a position `content relativism', though \citet{Cappelen2008} uses that term for a slightly different position. In \citet{MacFarlane2005-MACMSO} he discusses `assessment indexicality', a property sentences have if they express different propositions in different contexts. So there doesn't seem to be a settled terminology for this corner of the table, and I propose to take `indexical relativism' for it.
...
As \citet{MacFarlane2009-MACNC} argues, the felicity of homophonic reports does not raise a problem for either kind of non-indexical theory. I'll argue that it also doesn't pose a problem for indexical relativism.
...
So I think the indexical contextualist has no explanation of this agreement phenomena. But the indexical relativist has no such problem. When Clarke's utterance is being judged by Rebus, it expresses the proposition C(\textit{p}, \textit{q}, \textit{R(}Rebus)), so \(\neg\){}(\textit{p} \& \(\neg\){}\textit{q}) is clearly one of the propositions in the third clause. That's why agreement with another's utterance of a conditional is so easy.
...
The second option is to say that just uttering a conditional, \textit{If A, B}, adds \textit{A} to the background information. This seems like a bad option. For one thing, there is no independent reason to believe that this is true. For another, it can't explain what is wrong with the following kind of argument.
...
But why should we give any privilege to \textit{overt }indexicals? Tacit variables can be just as important in determining which form an argument takes. For example, the following argument is, on the most natural interpretation of each sentence, invalid.
...
\section*{Coda: Methodological Ruminations}
...
%Adams, Ernest (1998) \textit{A Primer on Probability Logic}, Palo Alto: CSLI.

%Bradley, Richard (2000) ``A Preservation Condition for Conditionals'' \textit{Analysis} 60: 219-222.

%Einheuser, Iris (2007) ``Three Forms of Truth-Relativism'', in M.Garcia-Carpintero and M.Kolbel (eds.), \textit{Relativising Utterance Truth}, Oxford: Oxford University Press, forthcoming.

%Gibbard, Allan (1981) ``Two Recent Theories of Conditionals'' in William Harper, Robert Stalnaker and Glenn Pearce (eds) \textit{Ifs}, Dordrecht: Reidel, pp. 211-47.

%Gillies, Anthony (ms) ``On truth-conditions for \textit{if} (but not quite only \textit{if} )'' 

%Grice, H Paul (1989) \textit{Studies in the Way of Words}, Cambridge, MA: Harvard University Press.

%Kaplan, David (1989) ``Demonstratives'' in \textit{Themes from Kaplan}, ed. by. Joseph Almog, John Perry, and Howard Wettstein. New York:. Oxford University Press, 481-563.

%King, Jeffrey and Jason Stanley (2005) ``Semantics, Pragmatics, and the Role of Semantic Content'', in \textit{Semantics vs. Pragmatics}, Z. Szab\'{o}, ed. (Oxford: Oxford University Press, 2005) (with Jeffrey C. King): 111-164.

%Kölbel, Max (2004) ``Indexical Relativism vs Genuine Relativism'' \textit{International Journal of Philosophical Studies} 12 (Oct. 2004), pp. 297--313.
...
%Lewis, David (1973) \textit{Counterfactuals}, Oxford: Blackwell.

%López de Sa, Dan (2007a) ``The Many Relativisms and the Question of Disagreement'', forthcoming in \textit{International Journal of Philosophical Studies }15.

%{}-{}-{}-{}- (2007b) ``Presuppositions of Commonality'', in M.Garcia-Carpintero and M.Kolbel (eds.), \textit{Relativising Utterance Truth}, Oxford: Oxford University Press, forthcoming.

%{}-{}-{}-{}- (ms) ``(Indexical) Relativism about Values: A Presuppositional Defense''

%Lycan, William (1993) ``MPP, RIP'' \textit{Philosophical Perspectives}

%MacFarlane, John (2003) ``Future Contingents and Relative Truth'' \textit{The Philosophical Quarterly} 53 (2003): 321-36

%{}-{}-{}-{}-{}- (2005) ``Making Sense of Relative Truth'' \textit{Proceedings of the Aristotelian Society} 105 (2005): 321-39

%{}-{}-{}-{}-{}- (2007) ``Semantic Minimalism and Nonindexical Contextualism'' in \textit{Context-Sensitivity and Semantic Minimalism: Essays on Semantics and Pragmatics}, ed. G. Preyer and G. Peter, Oxford: Oxford University Press

%{}-{}-{}-{}-{}- (ms) ``Non-indexical Contextualism''

%McGee, Vann (1985) ``A Counterexample to Modus Ponens'', \textit{Journal of Philosophy} 82: 462\nobreakdash-71.

%McKay, Thomas (2006) \textit{Plural Predication}. Oxford: Oxford University Press.

%Nolan, Daniel (2003) ``Defending a Possible-Worlds Account of Indicative Conditionals'' \textit{Philosophical Studies} 116: 215-69

%Partee, Barbara H. (1989) ``Binding Implicit Variables in Quantified Contexts'' in \textit{Papers from the Twenty-fifth Regional Meeting of the Chicago Linguistic Society}, Caroline Wiltshire, Randolph Graczyk, and. Bradley Music (eds.), 342--356. Chicago: Chicago Linguistic Society. Reprinted in Partee 2004.

%Partee, Barbara H. (2004) \textit{Compositionality in Formal Semantics: Selected Papers by Barbara H. Partee}. Oxford: Blackwell Publishing.

%Sayre-McCord, Geoffrey (1991) ``Being a Realist about Relativism (in Ethics)'' \textit{Philosophical Studies }61:155-176.

%Schlenker, Philippe (2003) ``Indexicality, Logophoricity, and Plural Pronouns'' in \textit{Research in }\textit{Afroasiatic Grammar II }(Selected Papers from the Fifth Conference on Afroasiatic Languages, Paris, 2000), ed. J. Lecarme, Benjamins, pp. 409-428

%Stalnaker, Robert (1975) ``Indicative Conditionals'' \textit{Philosophia }5: 269-286.

%Stalnaker, Robert (1981), ``A Defense of Conditional Excluded Middle'' in \textit{Ifs: Conditionals, Belief, Decision, Chance, and Time}, edited by William Harper, Glenn Pearce and Robert Stalnaker, Dordrecht: D. Reidel, 1981, pp 87-104.

%Stanley, Jason (2007) \textit{Language in Context: Selected Essays}, Oxford: Oxford University Press.

%Stephenson, Tamina (forthcoming) ``Judge Dependence, Epistemic Modals, and Predicates of Personal Taste'', forthcoming in \textit{Linguistics and Philosophy}.
