\chapter[Dogmatism and Intuitionistic Probability]{Dogmatism and Intuitionistic Probability auth=David Jehle, Brian Weatherson}

\addtocontents{toc}{\hspace{15pt}Co-authored with David Jehle \protect\par}

\pubdata{Greg Restall and Gillian Russell (eds.), \textit{New Waves in Philosophical Logic}, Palgrave, 2012, 95-111.}


\newcommand{\ModelPicture}{
\begin{center}
...
\newcommand{\TheoremTwo}{
\begin{quote}
\textbf{Theorem 2} \\
If $Pr$ is a classical probability function, then \begin{itemize}
\item $Pr(\neg(Ap \wedge \neg p) | Ap) \leq Pr(\neg(Ap \wedge \neg p))$; and 
...
\newcommand{\TheoremThree}{
\begin{quote}
\textbf{Theorem 3}.\\
In $M$, for any $x \in (0, 1)$,
\renewcommand{\labelenumi}{(\alph{enumi})}
\begin{enumerate}
\item $Pr_x(Ap \rightarrow p)$ = $Pr_x((Ap \rightarrow p) \wedge Ap) = x$
...
\newcommand{\TheoremFour}{
\begin{quote}
\textbf{Theorem 4}.\\
For any $x \in (0, 1)$,
\renewcommand{\labelenumi}{(\alph{enumi})}
\begin{enumerate}
\item $1 = Pr_x(Ap \rightarrow p | Ap) >  Pr_x(Ap \rightarrow p) = x$
\item $1 = Pr_x(\neg Ap \vee p | Ap) >  Pr_x(\neg Ap \vee p) = x$
\item $1 = Pr_x(\neg(Ap \wedge \neg p) | Ap) >  Pr_x(\neg(Ap \wedge \neg p)) = x$
\end{enumerate}
\end{quote}
...
%\begin{quote}
%\textbf{Theorem 5} \\
%Let $Pr_1$ and $Pr_2$ be regular classical conditional probability functions with the same domain, and let $T$ be any classical theorem. Let $f$ be defined as
%\begin{equation}
%f(p | q) = \frac{x Pr_1(p \wedge q | T) + (1-x) Pr_2(p \wedge q | T)}{x Pr_1(q | T) + (1-x) Pr_2(q | T)}
%\end{equation}
%Then $f$ is also a regular classical conditional probability function.
%\end{quote}
%}
...
%\begin{quote}
%\textbf{Theorem 6} \\
%Let $Pr_1$ and $Pr_2$ be classical conditional probability functions with a common and finite domain. Then the function $f_x$ defined in Appendix B is a classical conditional probability function. Moreover, if $x \in (0, 1)$, and $Pr_1(p | q) < Pr_2(p | q)$, then $Pr_1(p | q) < f(p | q) < Pr_2(p | q)$.
%\end{quote}
%}
...
\section{Intuitionistic Probability}
...
Now if $r \vdash_{IL} p$, then $r \wedge p ~_{IL}\dashv \vdash_{IL} p$, so $Pr_3(r \wedge p) = Pr_3(p)$, so $Pr_3(p | r) = 1$, as required for \textbf{(P5)}.
